...
class BootCompletedReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        Log.d(TAG, "BOOT_COMPLETED received. Restoring FocusForge services, goal reminders, and session state.")
        ...
        CoroutineScope(Dispatchers.Default).launch {
            try {
                ...
                // Reschedule all enabled goal reminders
                val goals = app.database.goalTemplateDao().getAllGoalTemplatesList()
                GoalReminderScheduler.rescheduleAllEnabledReminders(context.applicationContext, goals)
                ...
                Log.e(TAG, "Error restoring session/reminders on boot: ${e.message}", e)
            } finally {
